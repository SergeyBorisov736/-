...
Временная сложность: O(n²) во всех случаях (лучшем, среднем и худшем), где n - количество элементов в массиве. Это связано с тем, что алгоритм всегда выполняет двойной цикл, независимо от исходного порядка элементов.Объяснение: Алгоритм всегда выполняет двойной цикл, независимо от исходного порядка элементов. Количество сравнений: (n-1) + (n-2) + ... + 1 = n(n-1)/2 = O(n²).
...
Временная сложность: O(n²) в худшем и среднем случаях, O(n) в лучшем случае (когда массив уже отсортирован). Объяснение: В худшем случае (массив отсортирован в обратном порядке) алгоритм выполняет n(n-1)/2 сравнений и обменов.
...
Временная сложность: O(n²) в худшем и среднем случаях, O(n) в лучшем случае (когда массив уже отсортирован).Объяснение: В худшем случае (обратно отсортированный массив) для каждого элемента требуется сдвиг всех предыдущих элементов.


4.Сортировка слиянием —алгоритм состоит в разделении массива пополам,сортировке половин и их слиянии C++.Алгоритм использует стратегию "разделяй и властвуй", рекурсивно разделяя массив на две половины до тех пор, пока не останутся подмассивы из одного элемента, затем объединяет их в отсортированном порядке.
Объяснение работы алгоритма: 
...
Временная сложность: O(n log n) во всех случаях (лучшем, среднем и худшем).Объяснение: Массив делится пополам (log n уровней), на каждом уровне выполняется O(n) операций при слиянии. 

5.Сортировка Шелла — является модификацией сортировки вставками,сортирует между собой элементы, стоящие на местах, кратных определённому шагу.Улучшенная версия сортировки вставками, которая сортирует элементы, находящиеся на определенном расстоянии друг от друга (шаге), постепенно уменьшая этот шаг до 1.
Объяснение работы алгоритма: 
...
Временная сложность: O(n²) в худшем случае, O(n log n) в лучшем случае. Объяснение: Зависит от выбора последовательности шагов. В худшем случае может деградировать до O(n²).

6.Быстрая сортировка —один из самых известных и широко используемых алгоритмов сортировки. Алгоритм состоит в выборе опорного элемента, разделении массива на две части относительно опорного (одна — все элементы,меньшие опорного элемента, вторая — большие), и в сортировке полученных частей рекурсивным вызовом себя от них.Эффективный алгоритм, который выбирает опорный элемент и разделяет массив на две части: элементы меньше опорного и элементы больше опорного, затем рекурсивно применяет этот же алгоритм к обеим частям.
Объяснение работы алгоритма: 
...
Временная сложность: O(n log n) в среднем случае, O(n²) в худшем случае. Объяснение: В худшем случае (уже отсортированный массив и неудачный выбор опорного элемента) глубина рекурсии достигает O(n).
7.Пирамидальная сортировка —алгоритм строит кучу из исходного списка, затем постепенно извлекает наибольший элемент из кучи и помещает его в конец списка.Алгоритм, который преобразует массив в двоичную кучу (max-heap), затем многократно извлекает максимальный элемент из кучи и помещает его в конец массива.
Объяснение работы алгоритма: 
...
Временная сложность: O(n log n) во всех случаях.Объяснение: Построение кучи занимает O(n), каждый из n извлечений максимума требует O(log n) времени. 
...
Временная сложность: O(n) в худшем случае, O(1) в лучшем случае.Объяснение: В худшем случае требуется проверить все n элементов массива.
...
Временная сложность: O(log n) во всех случаях.Объяснение: На каждом шаге область поиска уменьшается вдвое, поэтому максимальное количество шагов = log₂n.
...
Временная сложность: O(log log n) в среднем случае, O(n) в худшем случае.Объяснение: Эффективен при равномерном распределении данных. В худшем случае (неравномерное распределение) деградирует до O(n).
...
Временная сложность: O(log n) во всех случаях.Объяснение: Алгоритм всегда делит массив в золотом сечении, что гарантирует O(log n) время выполнения. 
